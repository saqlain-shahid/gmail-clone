...
const Sidebar = () => {
  ...
  return (
    <div className=' w-[15%]'>
        <div className='p-3'>
            <button onClick={() => setOpen(true)} className=' flex items-center gap-2 p-4 rounded-2xl hover:shadow-md bg-[#C2E7FF]'>
                <LuPencil size={'24px'}/>
                Compose
            </button>
        </div>
        <div className=' text-gray-500'>
            {
                sidebarItems.map((item, idx) => {
                    return (
                        <div key={idx} className='flex items-center gap-4 pl-6 py-1 rounded-r-full hover:cursor-pointer hover:bg-gray-200 my-2'>
                        {item.icon}
                        <p>{item.text}</p>
                    </div>
                    )
