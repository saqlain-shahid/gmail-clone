            ...
            <div className='flex items-center gap-1'>
                {
                    mailType.map((item,idx) => {
                        return (
                            <button 
                             className={`${mailTypeSelected === idx ? `border-b-4 border-b-blue-600 text-blue-600` : `border-b-4 border-b-transparent`} w-52 hover:bg-gray-100 flex items-centergap-5 p-4`}
                             onClick={()=>setMailTypeSelected(idx)}
                             key={idx}>
                                {item.icon}
                                <span>{item.text}</span>
                            </button>
